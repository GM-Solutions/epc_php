...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        /* load db group accoring to  country */
        $country = !empty($this->post('country')) ? $this->post('country') : "india";        
        $all_group = $this->config->item('db_group');
        if (array_key_exists($country, $all_group)) {
            $this->load->database($all_group[$country]);
        } else {
            $this->load->database();
        }
    }
