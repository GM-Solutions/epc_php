...
class Bikemodels extends REST_Controller
{
    ...
    function dashboard_models_post()
    {
        ...
        if (!empty($sku_code)) {
            $this->db->where('sku.sku_code', $sku_code);
        }
        ...
        $all_prt['banner_data']['image_url'] = "http://via.placeholder.com/300x50";
    ...
    public function plate_details_post()
    {
        ...
        group by bpc.part_number order by bpc.href";
        //        echo $sql; die;
    ...
    public function view_search_details_post()
    {
        $search_type = $this->post('search_type');
        $id = $this->post('id');
        $vertical_id = $this->post('vertical_id');

        /*vertical for  MC And search type => sku (search result with model name)*/
    }
